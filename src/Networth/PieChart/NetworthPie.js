...
export default class NetworthPie extends Component {
  render() {
    let debts =
      parseInt(this.props.credit) +
      parseInt(this.props.loans) +
      parseInt(this.props.otherDebt);
    let assets =
      parseInt(this.props.investments) +
      parseInt(this.props.savings) +
      parseInt(this.props.otherAssets);
    let total = assets + debts;
    let debtPercentage = (debts / total) * 100;
    let assetPercentage = (assets / total) * 100;
    const data = [
      { value: debtPercentage, name: "Debts " },
      { value: assetPercentage, name: " Assets" },
    ];
    const height = 200;
    const width = 200;
      ...
      <>
        ...
        {total === 0 ? (
          <div className="report">
            <br />
            <i>Add values below to see a breakdown</i>
          </div>
        ) : (
          <div>
            {" "}
