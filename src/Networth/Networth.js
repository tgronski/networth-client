...
export default class Networth extends Component {
  ...
  handleDeleteEntry = () => {
    CalculationApiService.getCalculations()
      .then((res) => this.setState({ entries: [res], error: "", total: "" }))
      .then((res) => {
        if (
          this.state.entries[0] != null &&
          this.state.entries[0].length < 12
        ) {
  ...
  handleSubmit(e) {
    ...
    if (this.state.entries[0] != null && this.state.entries[0].length >= 12) {
      this.setState({
        error: `The maximum historical networth is 12. Please delete some of your past entries below`,
        total_disabled: true,
        total: "",
      });
    } else
      Promise.all([
        CalculationApiService.postCalculations(
          networth_total,
            ...
            <div id="Portfolio" className="profile">
              ...
              <div className="test">
                <div className="WalletTitle">
                  <NetworthPie
                    credit={this.state.CreditCardDebt}
                    loans={this.state.Loans}
                    savings={this.state.Savings}
                    investments={this.state.InvestmentsStocksBonds}
                    otherAssets={this.state.otherAssets}
                    otherDebt={this.state.otherDebt}
                  />
                </div>
                  ...
                  <div className="WalletForm">
                    {wallets.length > 0 ? (
                      <div className="WalletList">
                        {wallets.map((wallet) => (
                          <div key={wallet.id}>
                            <label htmlFor={wallet.wallet_categories}>
                              <FontAwesomeIcon
                                className="icon"
                                icon={this.icons(wallet.icon)}
                              />{" "}
                              {wallet.wallet_categories} $:{" "}
                            </label>
                    ...
                    {this.state.showDescription ? (
                      <div id="description">
                        Assets indicate that they earn value over time, while
                        debts lose value OR need to be paid back over time.
                      </div>
                    ) : null}
